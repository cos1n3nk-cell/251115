...
let submenus = {
    '淡江大學': {
        items: ['教育科技學系'],
        links: { '教育科技學系': 'https://hackmd.io/@cosine6/H1zDbzRkbe' },
        visible: false
    }
};
...
function draw() {
    ...
    // 當 iframe 隱藏時，在主畫面顯示文字
    if (!iframeElem || iframeElem.style('display') === 'none') {
        let textAnimDuration = 90; // 動畫持續 90 幀
        let startY = height + 100; // 起始位置（畫面下方）
        let endY = height / 2;     // 結束位置（畫面中央）
        let currentY = endY;

        // 在動畫期間內計算 Y 座標
        if (sceneTimer < textAnimDuration) {
            let n = map(sceneTimer, 0, textAnimDuration, 0, 1); // 將時間正規化為 0-1
            let easedN = easeInOutExpo(n); // 套用 Easing 函式
            currentY = lerp(startY, endY, easedN); // 計算當前 Y 座標
        }

        push();
        textAlign(CENTER, CENTER);
        textFont('Huninn'); // 使用 CSS 中定義的字型
        textSize(80);
        fill('#b1d4ffff'); 
        text('淡江大學', width / 2, currentY);
        pop();
    }
...
function drawMenu() {
    ...
    let mainGap = 88;
    let itemHeight = 48;
    let subItemHeight = 60;
    let relMouseX = mouseX - menuX;

    for (let i = 0; i < menuItems.length; i++) {
        let menuItem = menuItems[i];
        let y = currentY;
        let mainItemHovered = relMouseX >= 0 && relMouseX <= menuWidth && mouseY >= y && mouseY <= y + itemHeight;

        // 處理子選單可見性
        if (submenus[menuItem]) {
            let submenu = submenus[menuItem];
            let subYStart = y + itemHeight;
            let subMenuHovered = relMouseX >= 0 && relMouseX <= menuWidth && mouseY >= subYStart && mouseY <= subYStart + submenu.items.length * subItemHeight;
            submenu.visible = mainItemHovered || subMenuHovered;
        }

        fill(mainItemHovered ? '#ffd166' : 255); // hover 顏色
        // 左內距 40
        text(menuItem, 40, y);
        currentY += mainGap;

        // 處理子選單邏輯
        if (submenus[menuItem] && submenus[menuItem].visible) {
            let submenu = submenus[menuItem];
            let subYStart = y + itemHeight; // 從主選項下方開始

            // 繪製子選單項目
            for (let j = 0; j < submenu.items.length; j++) {
                let subItemY = subYStart + j * subItemHeight;
                let subItemHovered = relMouseX >= 0 && relMouseX <= menuWidth && mouseY >= subItemY && mouseY <= subItemY + subItemHeight;
                
                fill(subItemHovered ? '#ffd166' : 255);
                // 子選單縮排
                text(submenu.items[j], 60, subItemY + (subItemHeight - menuTextSize) / 2);
    ...
    if (mouseX >= menuX && mouseX <= menuX + menuWidth) {
        ...
        for (let i = 0; i < menuItems.length; i++) {
            ...
            if (submenus[menuItem] && submenus[menuItem].visible) {
                ...
                for (let j = 0; j < submenu.items.length; j++) {
                    let subItemY = subYStart + j * subItemHeight;
                    if (mouseY >= subItemY && mouseY <= subItemY + subItemHeight) {
                        showIframe(submenu.links[submenu.items[j]]);
                        return; // 點擊後結束函式
                    }
                    currentY += subItemHeight;
                }
